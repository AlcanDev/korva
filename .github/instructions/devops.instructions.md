...
// 1. LoggerInterceptor (shared lib from @df-libs) — logs all requests/responses
// 2. DataDog APM tracer initialized BEFORE any imports (main.ts)
import './tracer';  // ← must be first import

// tracer.ts
import tracer from 'dd-trace';
tracer.init({
  service: process.env.DD_SERVICE || 'insurance-bff',
  env: process.env.DD_ENV || 'qa',
  version: process.env.DD_VERSION || '1.0.0',
});
export default tracer;
```

**SLO targets (enforce in Datadog monitors):**
