...
# This is the ONLY local release command. All real releases go through
# the PR pipeline: feature PR → CI → merge → release-please accumulates
# commits → Release PR → review → merge → tag → GoReleaser.
release-dry:
...
help:
	...
	@echo "  make release-dry       Dry-run goreleaser locally (no publish)"
	@echo "  (real releases: merge the Release PR that release-please creates)"
