...
- Keywords: gitlab, pipeline, harbor, hub.fif.tech, vault, hcl, npm ci, USER node, configurable-pipelines, fif/dx, multi-stage, secret
- Tasks: creating a CI pipeline, writing a Dockerfile, managing secrets, configuring a build job, deploying to Kubernetes

## Context
All Falabella Financiero pipelines consume reusable pipeline definitions from the internal `fif/dx/configurable-pipelines` project (v5+). Docker images are pulled from and pushed to the internal Harbor registry at `hub.fif.tech`. Build-time configuration uses GitLab CI variables; runtime application secrets are injected by HashiCorp Vault Agent into the Kubernetes pod — never baked into the image or listed in `.gitlab-ci.yml`.
...
# .gitlab-ci.yml
include:
  - project: 'fif/dx/configurable-pipelines'
    ref: 'v5'
    file:
      - '/templates/node-bff.gitlab-ci.yml'

variables:
  APP_NAME: home-api-cl
  NODE_VERSION: "20"
  HARBOR_REGISTRY: hub.fif.tech
  IMAGE_NAME: hub.fif.tech/home/home-api-cl
```
