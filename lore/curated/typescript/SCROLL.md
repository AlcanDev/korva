...
const INSURANCE_PROVIDERS = {
  CL: { baseUrl: 'https://cigo.cl/api', timeout: 5000, retries: 3 },
  PE: { baseUrl: 'https://cigo.pe/api', timeout: 8000, retries: 2 },
  CO: { baseUrl: 'https://cigo.co/api', timeout: 6000, retries: 3 },
...
Validate at the infrastructure boundary. Do not trust CIGO/TRO/Apigee response shapes.

```typescript
// infrastructure/adapters/schemas/cigo-offers.schema.ts
import { z } from 'zod';

export const CigoOfferSchema = z.object({
  id:        z.string().min(1),
  name:      z.string(),
...
export const CigoOffersResponseSchema = z.object({
  offers: z.array(CigoOfferSchema),
  total:  z.number().int().nonnegative(),
});

export type CigoOffersResponse = z.infer<typeof CigoOffersResponseSchema>;

// In adapter
const raw = await this.httpService.get<unknown>(url, { headers });
const parsed = CigoOffersResponseSchema.safeParse(raw.data);

if (!parsed.success) {
  throw new ExternalApiValidationError('CIGO', parsed.error.flatten());
}
...
const offer = CigoOfferSchema.parse(raw) as InsuranceOffer;
...
function processApiResponse(data: unknown): InsuranceOffer[] {
  const parsed = CigoOffersResponseSchema.parse(data);
