...
export default function AdminScrollsPrivate() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMsg(t.privateScrolls.savedOk)
      if (isNew) { setIsNew(false); setSelected(null) }
    } catch {
      setMsg(t.privateScrolls.saveFailed)
    }
  }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl">
      <PageHeader
        icon={<Lock size={17} />}
        iconColor="#a371f7"
        title={t.privateScrolls.title}
        description={t.privateScrolls.description}
        badge="Teams"
        badgeColor="purple"
        hint={{ command: 'korva lore list --team', label: t.privateScrolls.hintLabel }}
        actions={
          <button
            onClick={openNew}
          ...
          >
            <Plus size={13} />
            {t.privateScrolls.newScroll}
          </button>
        }
      />
      <InfoCallout title={t.privateScrolls.differenceTitle} variant="tip" collapsible id="private-scrolls-diff">
        <p>{t.privateScrolls.differenceBody}</p>
        <p className="mt-2 pt-2 border-t border-[#3fb95015]">
          <TermTooltip term={t.glossary.scrollTerm} definition={t.glossary.scrollDef}>
            {t.glossary.scrollTerm}
          </TermTooltip>
          {' · '}
          <TermTooltip term={t.glossary.vaultTerm} definition={t.glossary.vaultDef}>
            {t.glossary.vaultTerm}
          </TermTooltip>
          {' · '}
        ...
        <div className="rounded-lg border border-[#f85149] bg-[#f8514910] p-4 mb-4">
          <p className="text-[#f85149] text-xs">
            {String(error).includes('402')
              ? t.privateScrolls.errorTeams
              : t.privateScrolls.errorLoad}
          </p>
        </div>
      )}
        ...
        <div className="md:col-span-2">
          <div className="rounded-lg border border-[#21262d] bg-[#161b22] overflow-hidden">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-[#21262d]">
              ...
              <span className="text-[#e6edf3] text-sm font-medium">{t.privateScrolls.listHeader}</span>
            </div>
            ...
            <div className="divide-y divide-[#21262d]">
              ...
              {(!data?.scrolls || data.scrolls.length === 0) && !isLoading && !error && (
                <p className="px-4 py-3 text-xs text-[#484f58]">{t.privateScrolls.noScrolls}</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-3">
          {selected ? (
            <div className="rounded-lg border border-[#21262d] bg-[#161b22] overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-[#21262d]">
                <input
                  ...
                  placeholder={t.privateScrolls.namePlaceholder}
              ...
              <div className="p-4">
                <textarea
                  ...
                  rows={18}
                  placeholder={t.privateScrolls.contentPlaceholder}
                ...
                />
                {msg && (
                  <p className={`mt-2 text-xs ${msg === t.privateScrolls.savedOk ? 'text-[#3fb950]' : 'text-[#f85149]'}`}>
                    {msg}
                  </p>
                )}
              </div>
            ...
            <div className="rounded-lg border border-[#21262d] bg-[#161b22] p-8 text-center">
              ...
              <p className="text-[#484f58] text-xs">{t.privateScrolls.emptyState}</p>
            </div>
          )}
        </div>
      </div>
